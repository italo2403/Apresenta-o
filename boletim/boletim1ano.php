                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ...
                        $notas_sql = "SELECT d.nome AS disciplina, n.unidade, n.av1, n.av2, n.conceito, n.pos_noa,
                                             n.mencao_final_anual, n.total_faltas, n.mencao_final_pos_noa, n.resultado_final_anual
                                      FROM notas_1B n
                        ...
                        if ($notas_result->num_rows > 0) {
                            ...
                            foreach ($disciplinas as $disciplina => $unidades) {
                                ...
                                $final_anual = isset($unidades[1]['mencao_final_anual']) ? $unidades[1]['mencao_final_anual'] : '';
                                $total_faltas = isset($unidades[1]['total_faltas']) ? $unidades[1]['total_faltas'] : '';
                                $final_pos_noa = isset($unidades[1]['mencao_final_pos_noa']) ? $unidades[1]['mencao_final_pos_noa'] : '';
                                $resultado = isset($unidades[1]['resultado_final_anual']) ? $unidades[1]['resultado_final_anual'] : '';
                                echo "<td>{$final_anual}</td><td>{$total_faltas}</td><td>{$final_pos_noa}</td><td>{$resultado}</td></tr>";
                            }
                            echo '</tbody></table>';
                        } else {
